<template>
  <b-card no-body>
    ...
    <b-table-simple bordered responsive="sm" striped>
      <b-thead>
        <b-tr>
          ...
          >
            <span
              v-if="data.oblastid == 0 && data.regionid == 0"
            >
              {{ $t("OblastName") }}
            </span>
            <span
    ...
    <!-- <b-row>
      ...
      <b-button class="mb-2" v-if="data.oblastid != 0 && data.regionid != 0" variant="outline-danger" size="sm" @click="topOblastChange">
        <feather-icon icon="ArrowLeftIcon">  </feather-icon> {{ $t('back') }}
      </b-button>  
        </b-col>
      </b-row> -->
  ...
  methods: {
    ChangeOrg(item) {
      this.$router.push({
        ...
        query: { orgid: item.organizationid, tabIndex: 1 },
      });
    },
    ChangeBirthPeriod() {
    ...
    Refresh() {
      ...
      // }
      this.isBusy = true;
      ReportService.ChildUnderGuardianshipReport(this.data).then((res) => {
        this.items = res.data.data;
        this.data.totalRows = res.data.total;
        this.data.oblastid = res.data.oblastid
        this.data.regionid = res.data.regionid
        this.isBusy = false;
        this.getTodayDate();
        this.calculate();
      });
    ...
    Printer() {
      ...
      ReportService.ChildUnderGuardianshipPrint(this.data).then((res) => {
