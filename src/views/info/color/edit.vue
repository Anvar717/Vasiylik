      ...
      <b-row>
        <b-col sm="12" md="6" lg="6">
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-form-input :placeholder="$t('code')" v-model="Color.code" />
            </div>
          </div>
         <div class="form-group form-row">
...
export default {
  ...
  created() {
    ...
    this.show = true;
    ColorService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.Color = res.data;
      })
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.Color.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.Color.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.Color.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
