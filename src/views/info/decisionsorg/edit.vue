            ...
            <div class="col-sm-7">
             <b-input-group>
                <b-form-input
                  ...
                  v-model="DecisionsOrg.Shortname"
                />
...
export default {
  ...
  created() {
    ...
    this.show = true;
    DecisionsOrgService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.DecisionsOrg = res.data;
      })
      .catch((error) => {
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.DecisionsOrg.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.DecisionsOrg.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.DecisionsOrg.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      DecisionsOrgService.Update(this.DecisionsOrg)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `decisionsorg` });
        })
        .catch((err) => {
