            ...
            <div class="col-sm-7">
               <b-input-group>
                <b-form-input
                  ...
                  v-model="SchoolLegalForm.Shortname"
                />
          ...
          <div class="form-group form-row">
           ...
           <b-input-group>
                <b-form-input
                  ...
                  v-model="SchoolLegalForm.Fullname"
                />
                <b-input-group-append>
          ...
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <v-select
                ...
                label="name"
                v-model="SchoolLegalForm.Stateid"
              ></v-select>
            </div>
          </div>
        </b-card>
...
import SchoolLegalFormService from "@/services/info/schoollegalform.service";
...
export default {
  ...
  created() {
    ...
    this.show = true;
    SchoolLegalFormService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.SchoolLegalForm = res.data;
      })
      .catch((error) => {
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.SchoolLegalForm.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.SchoolLegalForm.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.SchoolLegalForm.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      SchoolLegalFormService.Update(this.SchoolLegalForm)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `schoollegalform` });
        })
        .catch((err) => {
