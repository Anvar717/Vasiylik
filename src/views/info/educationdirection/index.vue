  ...
  <b-card no-body>
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button
            variant="primary"
            :to="{ name: 'Editeducationdirection',params : {id : 0}}"
            v-if="$store.state.auth.organizationtype !== 4 && $can('MicroTerritoryEdit','permissions')"
          >
    ...
    >
      ...
      <template #cell(actions)="{item}">
        <b-link :to="{ name: 'Editeducationdirection',params : {id : item.id}}"  v-b-tooltip.hover.top="$t('Edit')" style="margin-right : 5px">
...
export default {
  components : {
    BButton, BPagination, BTable, BCol, BRow, BSpinner, BCard, BTooltip, BBadge, BInputGroup, BFormInput, BInputGroupAppend, BLink, BModal, BCardText
  },
  directives: {
    'b-tooltip': VBTooltip,
    'b-modal': VBModal
  ...
  methods: {
    ...
    Delete(item){
      ...
      EducationDirectionService.Delete(item.id).then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      EducationDirectionService.GetList(
        this.filter.Search,
        this.filter.Sort,
        this.filter.Order,
