            ...
            <div class="col-sm-7">
              <b-input-group>
                <b-form-input
                  ...
                  v-model="PropertyRight.Shortname"
                />
            ...
            <div class="col-sm-7">
             <b-input-group>
                <b-form-input
                  ...
                  v-model="PropertyRight.Fullname"
                />
                <b-input-group-append>
...
export default {
  ...
  created() {
    ...
    this.show = true;
    PropertyRightService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.PropertyRight = res.data;
      })
      .catch((error) => {
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.PropertyRight.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.PropertyRight.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.PropertyRight.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      PropertyRightService.Update(this.PropertyRight)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `propertyright` });
        })
        .catch((err) => {
