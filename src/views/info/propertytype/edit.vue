          ...
          <div class="form-group form-row">
           ...
           <b-input-group>
                <b-form-input
                  ...
                  v-model="PropertyType.Shortname"
                />
...
export default {
  ...
  created() {
    ...
    this.show = true;
    PropertyTypeService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.PropertyType.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.PropertyType.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.PropertyType.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      PropertyTypeService.Update(this.PropertyType)
        .then((res) => {
