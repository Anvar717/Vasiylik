  ...
  <b-card no-body>
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button
            variant="primary"
            :to="{ name: 'EditModeratorAdmAppRejectReason',params : {id : 0}}"
          >
    ...
    >
      <template #cell(status)="{ item }">
        <b-badge
          :variant="item.status == 'Создан' ? 'light-primary' : 'light-success'"
        >
          {{ item.status }}
        </b-badge>
      </template>
      <template #cell(actions)="{ item }">
        <div class="text-center">
          <b-link :to="{ name: 'EditModeratorAdmAppRejectReason',params : {id : item.id}}" v-if="$store.state.auth.organizationtype !== 4"  v-b-tooltip.hover.top="$t('Edit')" style="margin-right : 5px;">
            <feather-icon icon="EditIcon"></feather-icon>
          </b-link>
          <b-link style="margin-right : 5px;" @click="$refs['DeleteModal'+item.id].show()" v-if="$store.state.auth.organizationtype !== 4" v-b-tooltip.hover.top="$t('Delete')">
...
<script>
import { BButton, BPagination, BTable, BCol, BRow, BSpinner, BCard, BTooltip, BBadge, BInputGroup, BFormInput, BInputGroupAppend, BModal, BLink, BCardText } from 'bootstrap-vue'
import ModeratorAdmAppRejectReasonService from "@/services/info/moderatoradmapprejectreason.service";
...
export default {
  ...
  methods: {
    ...
    Refresh() {
      this.isBusy = true;
      ModeratorAdmAppRejectReasonService.GetList(
        this.filter.Search,
        this.filter.Sort,
        this.filter.Order,
    ...
    Delete(item){
      ...
      ModeratorAdmAppRejectReasonService.Delete(item.id).then(res => {
