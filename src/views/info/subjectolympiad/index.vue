<template>
  <b-card no-body>
    ...
    >
      ...
      <template #cell(islocalolympiad)="{item}">
        <b-badge
          :variant="!item.islocalolympiad ? 'light-danger' : 'light-success'"
        >
          {{ item.islocalolympiad ? $t('yes') : $t('no') }}
        </b-badge>
      </template>
      <template #cell(isgroup)="{item}">
        <b-badge
          :variant="!item.isgroup ? 'light-danger' : 'light-success'"
        >
...
import SubjectOlympiadService from '@/services/info/subjectolympiad.service'
export default {
  components : {
  ...
  methods: {
    ...
    Refresh() {
      this.isBusy = true;
      SubjectOlympiadService.GetList(
        this.filter.Search,
        this.filter.Sort,
        this.filter.Order,
