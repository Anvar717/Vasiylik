          ...
          <div class="form-group form-row">
            ...
            <b-input-group>
                <b-form-input
                  ...
                  v-model="KinshipDegree.shortname"
                />
            ...
            <div class="col-sm-7">
             <b-input-group>
                <b-form-input
                  ...
                  v-model="KinshipDegree.fullname"
                />
                <b-input-group-append>
          ...
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <v-select
                ...
                label="name"
                v-model="KinshipDegree.genderid"
              ></v-select>
            </div>
          </div>
          <div class="form-group form-row">
...
export default {
  ...
  created() {
    ...
    this.show = true;
    KinshipDegreeService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.KinshipDegree = res.data;
      })
      .catch((error) => {
    ...
    HelperService.GetGenderList().then((res1) => {
      this.GenderList = res1.data;
    });
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.KinshipDegree.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.KinshipDegree.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.KinshipDegree.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      KinshipDegreeService.Update(this.KinshipDegree)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `kinshipdegree` });
        })
        .catch((err) => {
