      ...
      <b-col sm="12" md="6" lg="6">
        <b-card>
          ...
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-form-input :placeholder="$t('code')" v-model="Street.code" />
            </div>
          </div>
        </b-card>
      </b-col>
      <b-col sm="12" md="6" lg="6">
        <b-card>
          ...
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-form-input
                :placeholder="$t('streettype')"
                v-model="Street.streettype"
              />
            </div>
          </div>
          <div class="form-group form-row">
...
export default {
  ...
  created() {
    ...
    this.show = true;
    StreetService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.Street.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.Street.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.Street.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      StreetService.UpdateStreet(this.Street)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `street` });
        })
        .catch((err) => {
