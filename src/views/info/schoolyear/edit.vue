...
export default {
  ...
  created() {
    ...
    this.show = true;
    SchoolYearService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
  ...
  methods: {
    ...
    AddTranslate(){
      ...
      if(this.SchoolYear.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.SchoolYear.Translates.push(this.TranslateItem)
    ...
    GetTranslateItems(columnname){
      this.TranslateItems = this.SchoolYear.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      SchoolYearService.Update(this.SchoolYear)
        .then((res) => {
