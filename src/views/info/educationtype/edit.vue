        ...
        <b-col sm="12" md="6" lg="6">
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-form-input
                :placeholder="$t('code')"
                v-model="EducationType.code"
              />
            </div>
          </div>
          <div class="form-group form-row">
            <label class="col-form-label col-sm-5" for>{{ $t("shortname") }}</label>
            <div class="col-sm-7">
              <b-form-input
                :placeholder="$t('shortname')"
                v-model="EducationType.shortname"
              />
            </div>
          </div>
          <div class="form-group form-row">
            <label class="col-form-label col-sm-5" for>{{ $t("fullname") }}</label>
            <div class="col-sm-7">
              <b-form-input
                :placeholder="$t('fullname')"
                v-model="EducationType.fullname"
              />
            </div>
          </div>
          <!-- <div class="form-group form-row">
          ...
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <v-select
                ...
                :placeholder="$t('ChooseBelow')"
                label="name"
                v-model="EducationType.parentid"
              ></v-select>
            </div>
          </div>
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <v-select
                ...
                :placeholder="$t('ChooseBelow')"
                label="name"
                v-model="EducationType.qualificationcategoryid"
              ></v-select>
            </div>
          </div>
        </b-col>
...
import EducationTypeService from "@/services/info/educationtype.service";
import QualificationCategoryService from "@/services/info/qualificationcategory.service";
...
export default {
  ...
  created() {
    ...
    this.show = true;
    EducationTypeService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
    ...
    EducationTypeService.GetAll(this.lang, true).then((res) => {
      this.ParentList = res.data;
    });
    QualificationCategoryService.GetAll(this.lang).then((res) => {
  ...
  methods: {
    ...
    AddTranslate() {
      ...
      if (
        this.EducationType.Translates.filter(
          (item) =>
      ...
      this.EducationType.Translates.push(this.TranslateItem);
    ...
    GetTranslateItems(columnname) {
      this.TranslateItems = this.EducationType.Translates.filter(
        (item) => item.columnname == columnname
      );
    },
    SaveData() {
      EducationTypeService.Update(this.EducationType)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `educationtype` });
        })
        .catch((err) => {
