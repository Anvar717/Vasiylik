      ...
      <b-row>
        <b-col sm="12" md="6" lg="6">
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-form-input :placeholder="$t('code')" v-model="ChildExpulsionType.code" />
            </div>
          </div>
         <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-input-group>
                <b-form-input
                  ...
                  v-model="ChildExpulsionType.shortname"
                />
            ...
            <div class="col-sm-7">
               <b-input-group>
                <b-form-input
                  ...
                  v-model="ChildExpulsionType.fullname"
                />
                <b-input-group-append>
          ...
          <div div class="form-group form-row mb-0">
            ...
            <div class="col-sm-7">
              <b-form-checkbox
                v-model="ChildExpulsionType.isdead"
                name="check-button1"
                switch
              >
              </b-form-checkbox>
            ...
            <label
              class="col-form-label col-sm-5 d-flex align-items-center"
              for
              >{{ $t("istoorphanage") }}</label
            >
            <div class="col-sm-7">
              <b-form-checkbox
                v-model="ChildExpulsionType.istoorphanage"
                name="check-button1"
                switch
              >
              </b-form-checkbox>
        ...
        <b-col sm="12" md="6" lg="6">
         <div class="form-group form-row">
            <label class="col-form-label col-sm-5" for>{{ $t("fromage") }}</label>
            <div class="col-sm-7">
              <b-form-input :placeholder="$t('fromage')" v-model="ChildExpulsionType.fromage" />
            </div>
          </div>
         <div class="form-group form-row">
            <label class="col-form-label col-sm-5" for>{{ $t("toage") }}</label>
            <div class="col-sm-7">
              <b-form-input :placeholder="$t('toage')" v-model="ChildExpulsionType.toage" />
            </div>
          </div>
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <v-select
                ...
                label="name"
                v-model="ChildExpulsionType.stateid"
              ></v-select>
            </div>
          </div>
        </b-col>
...
export default {
  ...
  created() {
    ...
    this.show = true;
    ChildExpulsionTypeService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.ChildExpulsionType = res.data;
      })
      .catch((error) => {
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.ChildExpulsionType.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.ChildExpulsionType.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.ChildExpulsionType.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      ChildExpulsionTypeService.Update(this.ChildExpulsionType)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `childexpulsiontype` });
        })
        .catch((err) => {
