  ...
  <b-card no-body>
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button
            variant="primary"
           :to="{ name: 'EditReturnadmapptype',params : {id : 0}}"
          >
    ...
    >
      <template #cell(status)="{ item }">
        <b-badge
          :variant="item.status == 'Утвержден' ? 'light-success' : item.status == 'Изменен' ? 'light-info' : item.status == 'Пассив' ? 'light-danger' : item.status == 'Не принято' ? 'light-danger' : item.status == 'Актив' ? 'light-success' : item.status == 'Отправлен' ? 'light-warning' : 'light-primary'"
        >
          {{ item.status }}
        </b-badge>
      </template>
      ...
      <template #cell(actions)="{ item }">
        <div class="text-center text-nowrap">
          <b-link   v-b-tooltip.hover.top="$t('Edit')" :to="{ name: 'EditReturnadmapptype',params : {id : item.id}}" style="margin-right : 5px">
            <feather-icon icon="EditIcon"></feather-icon>
          </b-link>
          
          <b-link   @click="$refs['DeleteModal'+item.id].show()" style="margin-right : 5px" v-b-tooltip.hover.top="$t('Delete')">
...
export default {
  ...
  methods: {
    ...
    Delete(item){
      ...
      ReturnadmapptypeService.Delete(item.id).then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      ReturnadmapptypeService.GetList(
        this.filter.Search,
        this.filter.Sort,
        this.filter.Order,
