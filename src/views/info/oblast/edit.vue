...
export default {
  ...
  created() {
    ...
    this.show = true;
    OblastService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.Oblast = res.data;
      })
      .catch((error) => {
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.Oblast.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.Oblast.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.Oblast.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
    ...
    // },
    SaveData() {
      OblastService.Update(this.Oblast)
        .then((res) => {
          this.$makeToast(this.$t("SaveSuccess"), "success");
          this.$router.push({ name: `oblast` });
        })
        .catch((err) => {
