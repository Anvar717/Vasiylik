      ...
      <b-col sm="12" md="6" lg="6">
        <b-card>
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-form-input :placeholder="$t('code')" v-model="Mfy.Code" />
            </div>
          </div>
          <div class="form-group form-row">
          ...
          <div class="form-group form-row">
            ...
            <div class="col-sm-7">
              <b-form-input :placeholder="$t('inn')" v-model="Mfy.Inn" />
            </div>
          </div>
        </b-card>
      </b-col>
...
export default {
  ...
  created() {
    ...
    this.show = true;
    MfyService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
        this.Mfy = res.data;
      })
  ...
  methods: {
    ...
    clearLangTable(columnname){
     ...
     AddTranslate(){
      ...
      if(this.Mfy.Translates.filter(item => item.languageid === this.TranslateItem.languageid && item.columnname == this.TranslateItem.columnname).length > 0){
        this.$makeToast(this.$t('AlreadySelectLang'),'danger')
        return false
      }
      this.Mfy.Translates.push(this.TranslateItem)
   ...
   OpenTranslateModal(columnname){
     ...
     GetTranslateItems(columnname){
      this.TranslateItems = this.Mfy.Translates.filter(item => item.columnname == columnname)
    },
    // check(){
    //     if (
