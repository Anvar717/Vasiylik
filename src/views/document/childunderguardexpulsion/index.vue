  ...
  <b-card no-body>
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button
            variant="primary"
            :to="{ name: 'EditChildUnderGuardExpulsion', params: { id: 0 } }"
          >
    ...
    >
      <template #cell(statusname)="{ item }">
        <b-badge
          :variant="item.statusid == 2 ? 'light-success' : 
          item.statusid == 3 ? 'light-danger' : 'light-primary'"
        >
          {{ item.statusname }}
        </b-badge>
      </template>
      <template #cell(actions)="{ item }">
        <div class="text-center">
          <b-link
            :to="{ name: 'EditChildUnderGuardExpulsion', params: { id: item.id } }"
            v-b-tooltip.hover.top="$t('Edit')"
            style="margin-right: 5px"
            v-if="$can('ChildUnderGuardExpulsionEdit' , 'permissions') && item.statusid != 2"
          >
          ...
          <b-link
            ...
            v-b-tooltip.hover.top="$t('Delete')"
             style="margin-right: 5px"
            v-if="$can('ChildUnderGuardExpulsionDelete' , 'permissions') && item.statusid != 2"
          >
            <feather-icon icon="Trash2Icon"></feather-icon>
          </b-link>
            <b-link
            :to="{ name: 'ViewChildUnderGuardExpulsion', params: { id: item.id } }"
...
export default {
  ...
  methods: {
    ...
    Refresh(){
        this.isBusy = true;
        ChildUnderGuardExpulsionService.GetList(
    ...
    Delete(item) {
      ...
      ChildUnderGuardExpulsionService.Delete(item.id)
        .then((res) => {
