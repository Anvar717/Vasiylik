    ...
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button
            variant="primary"
            v-if="$can('ChildRegistrationActEdit', 'permissions')"
            :to="{ name: 'CreateChildRegistrationAct' }"
          >
    ...
    >
      ...
      <template #cell(actions)="{ item }">
        <div class="text-center">
          <b-link
            :to="{ name: 'EditChildRegistrationAct', params: { id: item.id } }"
            v-b-tooltip.hover.top="$t('Edit')"
            style="margin-right: 5px"
            v-if="
              $can('ChildRegistrationActEdit', 'permissions') &&
              item.statusid !== 2 && item.dateofexpulsion == null
            "
          >
...
export default {
  ...
  methods: {
    ...
    Refresh() {
      this.isBusy = true;
      ChildRegistrationActService.GetList(
        this.filter.Search,
        this.filter.Sort,
        this.filter.Order,
    ...
    Delete(item) {
      ...
      ChildRegistrationActService.Delete(item.id)
        .then((res) => {
