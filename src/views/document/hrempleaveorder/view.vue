  ...
  <b-overlay :show="show">
    <b-row>
            ...
            <b-col sm="12" md="6" lg="4">
              <b-list-group>
                <b-list-group-item class="d-flex justify-content-between">
                  ...
                  <span>
                    <b> {{ HREmpLeaveOrder.detailinfo }} </b>
                  </span>
                </b-list-group-item>
              </b-list-group>
            </b-col>
      ...
      <b-col sm="12" md="3" lg="2">
        <b-button
          v-if="$can('HREmpLeaveOrderApprove', 'permissions') && HREmpLeaveOrder.CanApprove"
          block
        ...
        <b-button
          v-if="$can('HREmpLeaveOrderCancelApproval', 'permissions') && HREmpLeaveOrder.CanCancelApprove"
          block
    ...
    <b-row>
      <b-col sm="12" md="9" lg="10">
        <b-card>
          <b-row>
            <b-col
              v-for="(item, index) in HREmpLeaveOrder.Tables"
              :key="index"
              sm="12"
              md="6"
              lg="6"
            >
              <b-list-group>
                ...
                <b-list-group-item class="d-flex justify-content-between">
                  <span> {{ $t("forperiodstart") }}: </span>
                  <span>
                    <b> {{ item.forperiodstart }} </b>
                  </span>
                </b-list-group-item>
                <b-list-group-item class="d-flex justify-content-between">
                  <span> {{ $t("forperiodend") }}: </span>
                  <span>
                    <b> {{ item.forperiodend }} </b>
                  </span>
                </b-list-group-item>
              </b-list-group>
            </b-col>
...
export default {
  ...
  methods: {
    Refresh() {
      this.show = true;
      HREmpLeaveOrderService.GetDetail(this.$route.params.id)
        .then((res) => {
          this.show = false;
    ...
    Approve() {
      ...
      HREmpLeaveOrderService.Approve(this.HREmpLeaveOrder.id)
        .then((res) => {
    ...
    CancelApproval() {
      ...
      HREmpLeaveOrderService.CancelApproval(this.HREmpLeaveOrder.id)
        .then((res) => {
