...
export default {
  ...
  created() {
    ...
    this.show = true;
    HrRecallLeaveService.Get(this.$route.params.id)
      .then((res) => {
        this.show = false;
  ...
  methods: {
    GetpersonPersondata() {
      ...
      EmployeeManageService.GetEmployeeListForComponent(
        (this.filter.OnDate = this.HrEmpSendTrain.docdate),
    ...
    Refresh() {
      this.isBusy = true;
      EmployeeManageService.GetEmployeeListForComponent(
        (this.filter.OnDate = this.HrRecallLeave.docdate),
        this.filter.DepartmentID,
        this.filter.HREmpAppointOrderTypeID,
        this.filter.Search,
        this.filter.SortColumn,
    ...
    AddPersonData() {
      ...
      HREmpLeaveOrderService.GetAll(this.tabrow.personid).then((res) => {
        this.HrempleaveorderList = res.data;
      });
    ...
    AddData() {
      this.tabrow.departmentname =
        this.DepartmentList.length > 0
          ...
          : "";
      this.tabrow.hrempleaveorder =
        this.HrempleaveorderList.length > 0
          ? this.HrempleaveorderList.filter(
              (item) => this.tabrow.hrempleaveorderid === item.id
            )[0].name
          : "";
          
      if (this.editedIndex > -1) {
        Object.assign(this.HrRecallLeave.Tables[this.editedIndex], this.tabrow);
      } else {
        this.HrRecallLeave.Tables.push(this.tabrow);
      }
    ...
    HrempleaveorderChange(){
      this.tabrow.hrempleaveordertableid = this.HrempleaveorderList.filter(item => item.id == this.tabrow.hrempleaveorderid)[0].tableid
    },
    rowClass(item) {
    ...
    DocdateValue(value) {
      this.HrRecallLeave.docdate = value;
    },
    Openperson() {
