    ...
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button
            variant="primary"
            v-if="$can('ParentApplicationEdit', 'permissions')"
          >
    ...
    >
      ...
      <template #cell(actions)="{ item }">
        <div class="text-center">
          <b-link
            :to="{ name: 'EditParentApplication', params: { id: item.id } }"
            v-b-tooltip.hover.top="$t('Edit')"
            style="margin-right: 5px"
            v-if=" $can('ParentApplicationEdit', 'permissions') && item.statusname !== 'Утвержден'"
          >
          ...
          <b-link
            ...
            v-if="
              $can('ParentApplicationDelete', 'permissions') &&
...
import ParentApplicationService from "@/services/document/parentapplication.service";
...
export default {
  ...
  methods: {
    ...
    Refresh() {
      this.isBusy = true;
      ParentApplicationService.GetList(
    ...
    Delete(item) {
      ...
      ParentApplicationService.Delete(item.id) 
        .then((res) => {
