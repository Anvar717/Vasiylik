  ...
  <b-overlay :show="show">
    <b-row>
      ...
      <b-col sm="12" md="3" lg="2">
        <b-button
          v-if="$can('HREmpSickLeaveApprove', 'permissions') && HREmpSickLeave.CanApprove"
          block
        ...
        <b-button
          v-if="$can('HREmpSickLeaveCancelApproval', 'permissions') && HREmpSickLeave.CanCancelApprove"
          block
    ...
    <b-row>
      <b-col sm="12" md="9" lg="10">
        <b-card>
          <b-row>
            <b-col
              v-for="(item, index) in HREmpSickLeave.Tables"
              :key="index"
              sm="12"
              md="6"
              lg="6"
            >
              <b-list-group>
                ...
                <b-list-group-item class="d-flex justify-content-between">
                  <span> {{ $t("givendate") }}: </span>
                  <span>
                    <b> {{ item.givendate }} </b>
                  </span>
                </b-list-group-item>
                <b-list-group-item class="d-flex justify-content-between">
                  <span> {{ $t("givenorganization") }}: </span>
                  <span>
                    <b> {{ item.givenorganization }} </b>
                  </span>
                </b-list-group-item>
                <b-list-group-item class="d-flex justify-content-between">
                  <span> {{ $t("yearworkexp") }}: </span>
                  <span>
                    <b> {{ item.yearworkexp }} </b>
                  </span>
                </b-list-group-item>
                <b-list-group-item class="d-flex justify-content-between">
...
export default {
  ...
  methods: {
    Refresh() {
      this.show = true;
      HREmpSickLeaveService.GetDetail(this.$route.params.id)
        .then((res) => {
          this.show = false;
    ...
    Approve() {
      ...
      HREmpSickLeaveService.Approve(this.HREmpSickLeave.id)
        .then((res) => {
    ...
    CancelApproval() {
      ...
      HREmpSickLeaveService.CancelApproval(this.HREmpSickLeave.id)
        .then((res) => {
